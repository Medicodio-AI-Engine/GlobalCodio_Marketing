...
/* `_updatedAt` backs sitemap <lastmod> and the article `dateModified` - a real
   edit date, not the publish date, which matters for regulatory content that
   gets revised after publication. */
export async function getAllPostSlugs() {
  return client.fetch(
    `*[_type == "blogPost"]{ "slug": slug.current, "publishedAt": publishedAt, _updatedAt }`
  );
}
