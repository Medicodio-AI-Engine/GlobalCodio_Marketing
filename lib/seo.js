...
export const TWITTER_HANDLE = process.env.NEXT_PUBLIC_TWITTER_HANDLE || '@globalcodio';

/** Canonical external profiles - kept here so Organization.sameAs stays consistent everywhere. */
export const SOCIAL_PROFILES = [
  'https://www.linkedin.com/company/globalcodio',
];

/** Physical offices - powers Organization PostalAddress (local SEO + AEO entity grounding). */
export const OFFICES = [
  ...
  {
    '@type': 'PostalAddress',
    streetAddress: 'B-Block, 8th Floor, Brigade Tech Park, 134/1 Whitefield Main Road',
...
/** Founder entity - reused across Organization, About, and Person schema. */
export const FOUNDER = {
  '@type': 'Person',
  name: 'Umesh Vaidyamath',
  jobTitle: 'Founder & CEO',
  description:
    "Co-founded INSZoom in 1999 - the immigration industry's first cloud-based case management platform, which grew to serve 1,000+ law firms before its 2020 acquisition by Mitratech. Founded GlobalCodio in 2025.",
};
...
export const FAQ_SECURITY = [
  {
    question: 'What security certifications and audit reports does GlobalCodio hold?',
    answer:
      'GlobalCodio has completed a SOC 2 Type II examination, which produces an independent attestation report rather than a certificate. GlobalCodio is a product of Medicodio Inc., which holds ISO 27001 certification, and runs on the same infrastructure and under the same information security management system; the certificate is issued to Medicodio Inc. and is available on request. The platform also meets GDPR, UK GDPR, and CCPA/CPRA requirements, implements HIPAA-ready safeguards for immigration medical records, and follows ABA-aligned AI governance standards. Full compliance documentation is available to qualified prospects under NDA.',
  },
  {
  ...
  return {
    ...
    '@graph': [
      ...
      },
      {
        ...
        ...FOUNDER,
        worksFor: { '@id': orgId },
        sameAs: ['https://www.linkedin.com/company/globalcodio'],
      },
      {
        '@type': 'WebSite',
        '@id': websiteId,
...
<footer>
  <p>© 2026 GlobalCodio, operated by Medicodio Inc. · <a href="${SITE_URL}">www.globalcodio.ai</a> · <a href="mailto:${SUPPORT_EMAIL}">${SUPPORT_EMAIL}</a></p>
</footer>`;
}
...
/** Build a Service schema for a specific service/product page. */
export function buildServiceSchema({ name, serviceType, description, url }) {
  return {
    '@context': 'https://schema.org',
    '@type': 'Service',
    name,
    ...
    areaServed: 'Worldwide',
    audience: [
      { '@type': 'BusinessAudience', audienceType: 'Immigration law firms' },
      { '@type': 'BusinessAudience', audienceType: 'Corporate immigration departments' },
    ],
  };
}

/**
 * Build a BlogPosting/Article schema for a blog post.
 *
 * `image` is required by Google for Article rich results, so it falls back to the
 * site OG image rather than being omitted - a post with no featured image in
 * Sanity was previously ineligible for an image-enhanced result entirely.
 */
export function buildArticleSchema({ title, description, url, datePublished, dateModified, authorName, image }) {
  return {
    '@context': 'https://schema.org',
    '@type': 'BlogPosting',
    headline: title,
...
/** Build a BusinessEvent schema (used for conferences GlobalCodio attends). */
export function buildEventSchema({ name, startDate, endDate, locationName, addressLocality, addressRegion, url, status, format }) {
  const online = /online|webcast|virtual/i.test(format || '');
  return {
    '@context': 'https://schema.org',
    '@type': 'BusinessEvent',
    name,
    ...
    ...(url ? { url } : {}),
    organizer: { '@type': 'Organization', name: 'AILA', url: 'https://www.aila.org' },
    performer: { '@id': `${SITE_URL}/#organization` },
  };
}

/** Build a ContactPage / local-business style schema. */
export function buildContactPageSchema() {
  return {
    '@context': 'https://schema.org',
    '@type': 'ContactPage',
    url: `${SITE_URL}/contact`,
    name: 'Contact GlobalCodio',
    description: 'Contact GlobalCodio - book a demo, request a free tech audit, or ask about the managed immigration technology operation. Offices in San Ramon, California and Bangalore, India.',
    mainEntity: {
      '@type': 'Organization',
 ...
 *
 * The builders each emit a standalone `@context` so they can be used on their own;
 * inside a graph that key is redundant and belongs only at the top level, so it is
 * stripped from the child nodes here. Falsy entries are dropped, which lets callers
 * pass a conditional schema without guarding.
 */
export function buildSchemaGraph(...schemas) {
  return {
    '@context': 'https://schema.org',
    '@graph': schemas.filter(Boolean).map(({ '@context': _context, ...node }) => node),
  };
}

/**
 * Build a Next.js `metadata` object with a self-referencing canonical and OG url.
 * Keeps canonicals consistent (always absolute, www, no trailing-slash drift).
 *
 * Next.js merges `metadata` shallowly: a page that declares `openGraph` REPLACES
 * the root layout's object rather than extending it. So the image, type, siteName
 * and locale have to be restated here, or every page built with this helper ships
 * without an og:image - which is what shared links render from.
 */
export function buildPageMetadata({ path = '', title, description, keywords, ogTitle, ogDescription, ogImage }) {
  ...
  return {
    ...
    openGraph: {
      ...
      ...(ogDescription || description ? { description: ogDescription || description } : {}),
...
export const PAGE_SCHEMAS = {
  ...
  network: buildSchemaGraph(
    buildBreadcrumbSchema([
      { name: 'Home', url: SITE_URL },
      { name: 'CodioNetwork', url: `${SITE_URL}/network` },
    ]),
    buildServiceSchema({
      name: 'CodioNetwork - Global Immigration Service Provider Network',
      serviceType: 'Immigration service provider coordination',
      description:
        'A curated B2B network of certified immigration service providers worldwide - translators (40+ languages), USCIS-approved physicians, apostille and authentication services, foreign attorneys, specialized couriers, and consular coordinators - coordinated directly through CodioCMS.',
      url: `${SITE_URL}/network`,
    }),
  ),
  codioOps: buildSchemaGraph(
    buildBreadcrumbSchema([
      { name: 'Home', url: SITE_URL },
      { name: 'CodioOps', url: `${SITE_URL}/codioops` },
    ]),
    buildServiceSchema({
      name: 'CodioOps - Managed Case Management Operations',
      serviceType: 'Managed technology operations for immigration firms',
      description:
        'CodioOps is the dedicated team that configures, optimizes, and continuously tunes your CodioCMS platform to match exactly how your firm operates. Typical full configuration and migration takes approximately six weeks. Bundled with every GlobalCodio engagement.',
      url: `${SITE_URL}/codioops`,
    }),
  ),
  hrmsIntegration: buildSchemaGraph(
    buildBreadcrumbSchema([
      { name: 'Home', url: SITE_URL },
      { name: 'HRMS Integration', url: `${SITE_URL}/hrms-integration` },
    ]),
    buildServiceSchema({
      name: 'HRMS Integration for Immigration Case Management',
      serviceType: 'HR system integration',
      description:
        'GlobalCodio connects immigration case management directly to Workday, SAP SuccessFactors, BambooHR, ADP, and Rippling - so employee data flows automatically and firms meet the integration standard corporate clients require in RFPs.',
      url: `${SITE_URL}/hrms-integration`,
    }),
  ),
  rfpResponse: buildSchemaGraph(
    buildBreadcrumbSchema([
      { name: 'Home', url: SITE_URL },
      { name: 'RFP Response', url: `${SITE_URL}/rfp-response` },
    ]),
    buildServiceSchema({
      name: 'RFP Response Support for Immigration Law Firms',
      serviceType: 'RFP and security questionnaire response',
      description:
        'GlobalCodio drafts immigration law firms\' responses to complex technical, security, and compliance RFP questions - documenting SOC 2 Type II, ISO 27001, GDPR, HIPAA-aligned controls, encryption, disaster recovery, and AI governance. Draft responses typically delivered within 72 hours.',
      url: `${SITE_URL}/rfp-response`,
    }),
  ),
  itServices: buildSchemaGraph(
    buildBreadcrumbSchema([
      { name: 'Home', url: SITE_URL },
      { name: 'Services', url: `${SITE_URL}/it-services` },
    ]),
    buildServiceSchema({
      name: 'GlobalCodio Managed Immigration Technology Services',
      serviceType: 'Managed immigration technology operations',
      description:
        'Fully managed immigration technology services: technology audit, migration and onboarding, configuration, HRMS integration, customer support, IT support, RFP response, and ongoing managed operations. Typical migration completes in approximately six weeks.',
      url: `${SITE_URL}/it-services`,
    }),
  ),
  customerSupport: buildSchemaGraph(
    buildBreadcrumbSchema([
      { name: 'Home', url: SITE_URL },
      { name: 'Customer Support', url: `${SITE_URL}/customer-support` },
    ]),
    buildServiceSchema({
      name: 'GlobalCodio Customer Support',
      serviceType: 'Immigration-literate software support and onboarding',
      description:
        'Immigration-literate support staffed by people who understand H-1Bs, I-140s, I-485s, and USCIS deadlines. Whole-team onboarding, proactive success reviews, and a dedicated CSM for larger accounts. Support hours Monday to Friday, 4am to 5pm Pacific.',
      url: `${SITE_URL}/customer-support`,
    }),
  ),
  contact: buildSchemaGraph(
    buildBreadcrumbSchema([
      { name: 'Home', url: SITE_URL },
  ...
  events: buildSchemaGraph(
    buildBreadcrumbSchema([
      { name: 'Home', url: SITE_URL },
      { name: 'Events', url: `${SITE_URL}/events` },
    ]),
    buildEventSchema({
      name: 'AILA Annual Conference & Webcast 2026',
      startDate: '2026-06-17',
      endDate: '2026-06-20',
      locationName: 'Marriott Marquis San Diego Marina',
      addressLocality: 'San Diego',
      addressRegion: 'CA',
      url: 'https://www.aila.org/ac26',
      status: 'upcoming',
      format: 'In-person & online webcast',
    }),
    buildEventSchema({
      name: 'AILA California Chapters Conference 2026',
      startDate: '2026-11-05',
      endDate: '2026-11-07',
      locationName: 'Hyatt Regency San Francisco Downtown-SOMA',
      addressLocality: 'San Francisco',
      addressRegion: 'CA',
      url: 'https://www.aila.org/shop/products/view/california-chapters-conference',
      status: 'upcoming',
      format: 'In-person & online webcast',
    }),
  ),
